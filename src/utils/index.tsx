...
export const dateToString = (date: Date) => {
  ...
  let month = (date.getMonth() + 1 < 10 ? "0" : "") + (date.getMonth() + 1).toString();
...
export const scrollTo = (from: string, to: number | null, after?: () => void) => {
  var fromEl: any = document.querySelector(`.${from}`);
  if (fromEl !== null) {
...
export function viewSplitLine(message: string) {
  ...
  const replace = (message: string) => {
    ...
    const convertContent = convertContent1.replace(urlRegex, function (text: string) {
...
export function getUser(set: SetterOrUpdater<UserProps>, before?: any, after?: any) {
